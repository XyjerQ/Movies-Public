...
import * as apikey from '../API/apikey'
    ...
    fetch(`https://api.themoviedb.org/3/tv/${props.id}?language=en-US`, apikey.options)
    ...
    .then(response => {
        ...
        setTimeout(loadD, 750)
    })
    .catch(err => console.error(err));

    fetch(`https://api.themoviedb.org/3/tv/${props.id}/credits?language=en-US`, apikey.options)
    .then(response => response.json())
    .then(response => {
    ...
    fetch(`https://api.themoviedb.org/3/tv/${props.id}/videos`, apikey.options)
    .then(response => response.json())
    .then(response => {
    ...
    fetch(`https://api.themoviedb.org/3/tv/${props.id}/recommendations`, apikey.options)
    .then(response => response.json())
    .then(response => {
        console.log(response)
        for(i = 0; i < response.results.length; i++) {
    ...
    <div class="bg-dark text-light border-top border-light d-flex">
        <div class="container-fluid h-75 d-flex justify-content-center m-2">
            <div class="row my-3 w-100 border border-secondary rounded">
                <div class="col d-flex py-5 justify-content-center">
                    <img class="rounded" :src="'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/'+fetchDetails.poster" alt="" style="height: 800px;">
                    </div>
                <div class="col-7">
                    ...
                    <div v-else class="conteiner-fluid pt-5 ">
                        ...
                        <div>
                            <span>
                                ...
                                <span v-if="fetchDetails.seasons != 0">S{{ fetchDetails.seasons }}</span>
                                <span v-if="fetchDetails.episodes != 0"> E{{ fetchDetails.episodes }} ({{ fetchDetails.status }})</span>
                            </span>
                        </div>
                    </div>
                    <div class="my-2">
